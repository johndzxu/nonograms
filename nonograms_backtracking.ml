...
let rec verify_row (row: row) (clues: int list) =
  ...
  | (h::t, _) -> 
      if List.hd row = Black then 
      ...
      else if List.hd row = White then let c::cells = row in verify_row cells clues
      else
        (
          let (firstk, rest) = split h row in 
          if List.for_all (fun x -> (x = Black || x = Unknown)) (firstk) then ( 
        ...
        ) || let c::cells = row in verify_row cells clues
...
let rec ver_grid nono clues = 
  let clue_cols = List.mapi (fun i clue -> verify_col nono clue i) clues in
  List.for_all (fun clue -> clue) clue_cols
  
let rec ver_grid_with_rows nono row_cls col_cls = 
  let clue_cols = List.mapi (fun i clue -> verify_col nono clue i) col_cls in
  let clue_rows = List.mapi (fun i clue -> verify_row (List.nth nono i) clue) row_cls in
  List.for_all (fun clue -> clue) clue_cols && List.for_all (fun clue -> clue) clue_rows
...
let generate_children nono row_cls col_cls =
  ...
  List.filter (fun child -> (ver_grid_with_rows child row_cls col_cls)) all_children
...
let solve row_cls col_cls =  
  let rec s_row row_cls col_cls nono = 
    if ver_grid nono col_cls then 
      ...
      try 
        let children = generate_children new_nono row_cls col_cls in 
